...
/**
   Constructeur
 */
LocalSearch::LocalSearch(int nbVar, int lAssumption)
{    
  numatom = nbVar;
  ...
  bestAtom = new unsigned int [numatom + 1];
 ...
 */
void LocalSearch::initWithClauses(vec<CRef> &clauses, ClauseAllocator &ca)
{
...
/**
 *   clauses can be hard or soft
 */
void LocalSearch::initWithClauses(vec<CRef> &clauses, ClauseAllocator &ca, vec<int>& weights)
{
  assert(clauses.size() == weights.size());
  h_breakcount = new int[numatom + 1];
  h_makecount  = new int[numatom + 1];
  s_breakcount = new int[numatom + 1];
  s_makecount  = new int[numatom + 1];
  markFlipped  = new bool[numatom + 1];
  currentSizeDB = clauses.size();
  watch1 = (int *) realloc(watch1, sizeof(int) * clauses.size());
  watch2 = (int *) realloc(watch2, sizeof(int) * clauses.size());
  numtruelit = (int *) realloc(numtruelit, sizeof(int) * clauses.size());
...
*/
void LocalSearch::initInterpretation()
{
 ...
 */
void LocalSearch::initVariableRL(vec<CRef> &clauses, ClauseAllocator &ca)
{
  numfalse = 0;  
  recupScore = false;
 ...
 */
void LocalSearch::init(vec<CRef> &clauses, ClauseAllocator &ca)
{  
...
*/
void LocalSearch::flipatom(Lit toflip, vec<CRef> &clauses, ClauseAllocator &ca)
{     
  int numocc;
  int *occptr;
  int index;
 ...
 * bestLitToFlip
 */
Lit LocalSearch::bestLitToFlip(vec<CRef> &clauses, ClauseAllocator &ca)
{
  ...
  for(int i=0; i<=numatom; i++) if(!isAssign[i] && (h_makecount[i]-h_breakcount[i] > 0)) best.push(i);
  while(best.size())
  {
	rand = random() % best.size();
	v = best[rand];
	for(int i=0; i<clauses.size(); i++)
	{
	  Clause &c = ca[clauses[i]];
	  for(int j=0; j<c.size(); j++) if(var(c[j]) == v) return c[j];
	}
	best[rand] = best.last();
	best.pop();
  }
  ...
  for(int i=0; i<=numatom; i++)
	  if(!isAssign[i] && (h_makecount[i]-h_breakcount[i] == 0) &&
	    (s_makecount[i]-s_breakcount[i] > 0) && (sscore < s_makecount[i]-s_breakcount[i]))
	  {v = i; sscore = s_makecount[i]-s_breakcount[i];}
  if(sscore)
  {
	for(int i=0; i<clauses.size(); i++)
	{
...
void LocalSearch::flipatom4PMS(Lit toflip, vec<CRef> &clauses, ClauseAllocator &ca)
{
  int numocc;
  int *occptr;
  int index;

 // printf("toflip : %s%d\n", sign(toflip) ? "-" : "", var(toflip)+1);

  numflip++;
  markFlipped[var(toflip)] = true;
  if((toflip == lit_Undef) || WASS_TRUE(toflip, atom[var(toflip)])){numnullflip++; return;}
  ...
  for(int i = 0 ; i < numocc ;i++)
      ...
      if (--(numtruelit[index]) == 0)
	{
	  ...
	  for (int j = 0 ; j < cl.size() ; j++) makecount[var(cl[j])] += weightClause[index];
//-------------------------------------------------------------------------------------
	  if(isHardClause(index))
	  {
		h_numfalse++;
		h_breakcount[var(toflip)] -= weightClause[index];
		for (int j = 0 ; j < cl.size() ; j++) h_makecount[var(cl[j])] += weightClause[index];
	  }
	  else
	  {
		s_numfalse++;
		s_breakcount[var(toflip)]--;
		for (int j = 0 ; j < cl.size() ; j++) s_makecount[var(cl[j])]++;
	  }
//--------------------------------------------------------------------------------------
	}
      else if(numtruelit[index] == 1)
	{
	  ...
	  breakcount[watch1[index]] += weightClause[index];
//-------------------------------------------------------------------------------------
	  if(isHardClause(index)) h_breakcount[watch1[index]] += weightClause[index];
	  else s_breakcount[watch1[index]]++;
//-------------------------------------------------------------------------------------
	}
      else /* numtruelit[cli] >= 2 */
	{
...
//-------------------------------------------------------------------------------------
      ...
      else if(numtruelit[index] ==  2)
	{
	  ...
	  breakcount[watch1[index]] -= weightClause[index];
//-------------------------------------------------------------------------------------
	  if(weightClause[index]) h_breakcount[watch1[index]] -= weightClause[index];
	  else s_breakcount[watch1[index]]--;
//-------------------------------------------------------------------------------------
	}
    }
...
/**
   Main wsat
 */
lbool LocalSearch::wsat(int numtry, vec<CRef> &clauses, ClauseAllocator &ca)
{
  init(clauses, ca);
  ...
  while(numfalse && numtry)
    {                 
      bornMaxFlips = super(numtry) * 500000;      
...
lbool LocalSearch::wsat_(int numtry, vec<CRef> &clauses, ClauseAllocator &ca)
{
  init(clauses, ca);
  if(!numfalse) return l_True;
  ...
  while(numfalse && numtry)
    {                 
      ...
      for(numflip = 0 ; numflip < bornMaxFlips && numfalse ; numflip++)
        { 
	  Lit p = best(clauses, ca);
	  //Lit p = rnovelty(clauses, ca);
	  flipatom(p, clauses, ca);
          update_statistics_end_flip();
        }
 ...
 * init_wsat4pms
 */
void LocalSearch::init_wsat4PMS(vec<CRef> &clauses, ClauseAllocator &ca)
{
  numfalse = h_numfalse= s_numfalse =  0;
  recupScore = false;
  totalAssignFalse = false;
  ...
  for(int i = 0 ; i <= numatom ; i++)
    {
      isLitFree[i] = 0;
      changed[i] = -1;
      breakcount[i] = makecount[i] = h_breakcount[i] = h_makecount[i] = s_breakcount[i] = s_makecount[i] = markFlipped[i] = 0;
    }
  ...
  for(int i = 0 ; i < clauses.size() ; i++)
      ...
      if(!(numtruelit[i]))
	{
	  ...
	  falseClause[numfalse++] = i;
	  for(int j = 0 ; j < cl.size() ; j++) makecount[var(cl[j])] += weightClause[i];
	  if(isHardClause(i))// this is a hard clause
	    {
	      h_numfalse++;
	      for(int j = 0 ; j < cl.size() ; j++) h_makecount[var(cl[j])] += weightClause[i];
	    }
	  else
	    {
	      s_numfalse++;
	      for(int j = 0 ; j < cl.size() ; j++) s_makecount[var(cl[j])] += 1;
	    }
	}
      else if(numtruelit[i] == 1)
	{
	  breakcount[var(truelit1)] += weightClause[i];
	  if(isHardClause(i)) h_breakcount[var(truelit1)] += weightClause[i];
	  else s_breakcount[var(truelit1)] += 1;
	  watch1[i] = var(truelit1);
	  if(cl.toTransfer()) isLitFree[var(truelit1)]++;
	}
      else /* if (numtruelit[i] == 2) */
...
lbool LocalSearch::wsat4PMS(int numtry, vec<CRef> &clauses, ClauseAllocator &ca)
{
  int cost = INT_MAX;
  init_wsat4PMS(clauses, ca);
  if(!numfalse) return l_True;
  ...
  while(numfalse && numtry)
    {
      ...
      for(numflip = 0 ; numflip < bornMaxFlips && numfalse ; numflip++)
        {
          flipatom4PMS(best(clauses, ca), clauses, ca);
    	  if((h_numfalse == 0)  && (s_numfalse < cost))
    	  {
  ...
  while(h_numfalse != 0)
  {
	flipatom4PMS(bestLitToFlip(clauses, ca), clauses, ca);
    if((h_numfalse == 0)  && (s_numfalse < cost))
    {
  	  for(int i = 0; i<=numatom; i++) bestAtom[i] = atom[i];
	  cost = s_numfalse;
...
/**
   Rnovelty heuristic
 */
Lit LocalSearch::rnoveltyFree(vec<CRef> &clauses, ClauseAllocator &ca, vec<bool> &cbs)
{
 ...
 */
Lit LocalSearch::tabu(vec<CRef> &clauses, ClauseAllocator &ca)
{  
...
/**
   The metaheuristic tabu
 */
Lit LocalSearch::best(vec<CRef> &clauses, ClauseAllocator &ca)
{  
  int numbreak;
  vec<Lit> safety;
 ...
 */
Lit LocalSearch::rnovelty(vec<CRef> &clauses, ClauseAllocator &ca)
{
  int diff, birthdate;
  Lit youngest = lit_Undef,  best = lit_Undef, second_best = lit_Undef;
  int youngest_birthdate, best_diff, second_best_diff;
 ...
 */
void LocalSearch::oneDescent(int nFlips, vec<CRef> &clauses, ClauseAllocator &ca, vec<Lit> &mt, vec<bool> &cbs)
{
  flippedLit.zero(); 
  int nbNotFind = 1;
...
*/
void LocalSearch::makeRotation(vec<CRef> &clauses, ClauseAllocator &ca, vec<int> &idx, vec<Lit> &coMss,
...
/**
 */
int LocalSearch::flipTransition(vec<CRef> &clauses, ClauseAllocator &ca, Lit l, vec<Lit> &cAss)
{
  int numocc;
  int *occptr;
  int index;
...
/**
   ...
   @param[in] cbs, flag to know if a selector can be selected
*/
void LocalSearch::makeRotation_acc(Lit l, vec<CRef> &clauses, ClauseAllocator &ca, vec<Lit> &mt,
...
bool LocalSearch::refineAssigns(bool optWSAT, vec<CRef> &clauses, ClauseAllocator &ca, vec<lbool> &assigns)
{
  //set interpretation
  assert(assigns.size() == numatom - 1);
  for(int i = 0; i<assigns.size(); i++)
    if(isAssign[i]) atom[i] = isAssign[i] & 1;
    else atom[i] = (assigns[i] == l_True)? true : false;
  for(int i = 0; i <= numatom; i++) bestAtom[i] = atom[i];

  init(clauses, ca);

  printf("c NB clauses falsified : %d\n",numfalse);

  if (optWSAT) { 
      printf("c Refine interpretation with WSAT method\nc\n");
      wsat_(1,clauses, ca);
      for(int i = 0; i <= numatom; i++) bestAtom[i] = atom[i];
    }
  else
    {
      printf("c Refine interpretation with one descend in local search\n");
      bool markedVars[assigns.size()];
      for(int i = 0; i<assigns.size(); i++) markedVars[i] = false;

      vec<Lit> litsToFlip;
      for(int i = 0; i<numfalse; i++)
	{
	  Clause& c = ca[clauses[falseClause[i]]];
	  for(int j = 0; j<c.size(); j++) 
	    if(!markedVars[var(c[j])] && (var(c[j]) < limitAssumption)) { 
	      litsToFlip.push(c[j]); 
	      markedVars[var(c[j])] = true;
	    }
	}

      for(int i = 0; i<litsToFlip.size(); i++)
	{
	  if(makecount[var(litsToFlip[i])] - breakcount[var(litsToFlip[i])] > 0)
	    {
	      flipatom(litsToFlip[i], clauses, ca);
	    }
	}
      for(int i = 0; i <= numatom; i++) bestAtom[i] = atom[i];
    }

  for(int i = 0; i<assigns.size(); i++) assigns[i] = (bestAtom[i])? l_True : l_False;

  printf("c NB clauses still falsified : %d\n",numfalse);
  return (numfalse == 0);
}//refineAssigns



void LocalSearch::refineSoftPartAssigns(bool optWSAT, vec<CRef> &clauses, ClauseAllocator &ca, vec<lbool> &assigns)
{
  //set interpretation
  assert(assigns.size() == numatom - 1);
  for(int i = 0; i<assigns.size(); i++) {
  ...
  for(int i = 0; i<numfalse; i++)
    {
      Clause& c = ca[clauses[falseClause[i]]];
      for(int j = 0; j<c.size(); j++)
	if(!isAssign[var(c[j])] && !markedVars[var(c[j])] && (var(c[j]) < limitAssumption))
	  { litsToFlip.push(c[j]); markedVars[var(c[j])] = true;}
    }
  for(int i = 0; i<litsToFlip.size(); i++) if(!breakcount[var(litsToFlip[i])]) flipatom(litsToFlip[i], clauses, ca);
  for(int i = 0; i<litsToFlip.size(); i++) assigns[var(litsToFlip[i])] = (atom[var(litsToFlip[i])])? l_True : l_False;
 }

 printf("c NB clauses still falsified : %d\n",numfalse);
}//refineSoftPartAssigns
...
*/
void LocalSearch::debugTest(vec<CRef> &clauses, ClauseAllocator &ca)
 {   
   static int cpt = 0;
